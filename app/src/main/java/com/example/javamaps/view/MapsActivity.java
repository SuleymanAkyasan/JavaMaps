...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback,GoogleMap.OnMapLongClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = Room.databaseBuilder(MapsActivity.this,PlaceDatabase.class,"Places")
                //allowMainThreadQueries() eklesek rxJavaya gerek kalmaz tertemiz kaydolur uygulama da çökmez
                //ancak önerilmez bu method...
                .build();
        placeDao = db.placeDao();
        selectedLatitude=0.0;
        selectedLongitude=0.0;

        binding.saveButton.setEnabled(false); // kullanıcı gerçekten bi yer seçmeden etkin hale gelmez
    }
    ...
    public void save(View view){
        ...
        // placeDao.insert(place); bu verimsiz

        // threading -> Main(UI), Default (CPU intensive), IO (network,database)

        //placeDao.insert(place).subscribeOn(Schedulers.io()).subscribe();
        //bu daha verimsiz alttaki çok daha iyi


        //disposable
        compositeDisposable.add(placeDao.insert(place)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()) //burada gözlemleme işlemi yapılır koymasakta çalışır.
                .subscribe(MapsActivity.this::handleResponse) //handleResponse'a referans verdik
        );
